...
- Web ダッシュボード（SKYLEDGER レーダーテーマ）複数ページ：
  ...
  - `/details`：過去の航空機コンタクト検索 / filter（会社・機種・**カテゴリ CAT**・路線・国・高度）+ sort。CAT は ADS-B emitter category のグループ（飛行機 / ヘリ / グライダー / 気球 / パラシュート / 超軽量機 / ドローン / 宇宙機 / 地上車両 / 障害物 / 未分類）。「残っている最古の raw」の日より古い日付（通常は 30 日 retention の外）は自動的に `aircraft_passes` から row を組み立てるので、古い日も同じように検索できる。ただし `sightings_raw` が空だと floor が取れず、fallback は働かない
  - `/stats`：7 日間の便数、直近 24 時間の毎時 histogram、**直近 30 日 weekday × hour heatmap**、TOP 10（機種 / 会社 / 出発 / 目的地 / **ICAO** 7 日 + 全 DB）、peak altitude、busiest hour；**記録・歴代ベスト**（最速 / 最長滞在の pass / 最も忙しかった日）、**通年カレンダー heatmap**（日別 pass 数、約 53 週）、**進行方位コンパス**（直近 7 日の track を 16 方位に分布）、**最速 TOP 10**、**対地速度の分布 histogram**；**ロングウィンドウ**：累計 unique ICAO 発見曲線、最高高度分布 histogram、レア機リスト（1〜2 回だけの ICAO）。ページ上部に snapshot の更新時刻を表示。日別 / 毎時グラフは**完了した期間のみ**（histogram は昨日から 7 日、24h は完了した 24 時間分）を表示し、途中の期間がゼロに落ちたように見えるのを回避。`/discover` 旧 URL は 301 redirect
...
6. `src/prune_raw.py`（耐障害、rc 非ゼロでも継続）：30 日より古い `sightings_raw` を削除

6 step の後にヘリ集結 alert（`query_live` のリアルタイム snapshot → detect → cooldown dedup → push）を実行。上の step とは独立。
...
- `manage.py ingest_pipeline`：6 step sequence（上記リスト）+ ヘリ集結 push
...
- `manage.py refresh_stats_cache`：`/api/stats` と `/api/discover` の永続 JSON snapshot をバックグラウンドで事前計算し、`data/stats-cache.json` へアトミックに書き込み（計算失敗時は旧ファイルを保持）
...
- `data/stats-cache.json`：`/api/stats` + `/api/discover` の毎時統計 snapshot（`refresh_stats_cache` が書き、web request は読むだけ）
...
JSON API：
- `/api/stats`：毎時 snapshot の統計データ（7 日 / 24h histogram、heatmap、top 10、記録、カレンダー、コンパス、速度分布、peak alt、busiest hour）
- `/api/discover`：同じ毎時 snapshot の discovery curve、rare finds、altitude 分布、全 DB top 10 ICAO（`/stats` ページはこれと `/api/stats` を同時 fetch）
- 両方とも `data/stats-cache.json` を**読むだけ**で、request 内で重い SQL は走らせない。snapshot 未生成 / バージョン不一致なら 503 `stats_cache_unavailable`、成功時は `X-Stats-Cache-Generated-At` header 付き（フロントが snapshot 時刻表示に使用）
...
- `/api/today?day=&sort=&country=&operator=&type=&cat=&from=&to=`：home + details ページ用、rows + filter dropdown options（`cat` = ADS-B emitter category グループ）
...
- Raw retention：`sightings_raw` は 30 日だけ保持（`src/prune_raw.py`、pipeline 最終 step）、`aircraft_passes` は永久保存。よって map track / profile chart は 30 日まで遡れるが、`/details` や統計は古い日付でも表示可（retention floor より古い日は passes から row を組み立て）。floor は `MIN(sightings_raw.seen_at)` から求めるので、raw table が空（未 ingest / 直後に全削除）なら floor 無し = fallback も無し
- `/api/live` は module-level dict cache（process-local、restart で即クリア、Redis 不要）
- `/api/stats` + `/api/discover` は file-backed snapshot（`data/stats-cache.json`）。gunicorn は読む前に mtime で reload 要否を判定、書き込みは `os.replace()` でアトミック置換なので書きかけ JSON を読むことはない
